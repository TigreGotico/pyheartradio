...
def test_search_stations(client):
    search_data = {
        "results": {
            "stations": [{"id": 1, "name": "Rock FM"}]
        }
    }
    station_data = {
        "hits": [{"streams": {"shoutcast_stream": "http://stream.example.com"}, "logo": "http://img.example.com", "description": "Best rock"}]
    }
...
def test_search_podcast(client):
    data = {
        "results": {
            "podcasts": [
                {"id": 42, "title": "Tech Talk", "image": "http://img.example.com", "description": "Tech news"}
            ]
        }
    }
...
def test_get_podcast_episodes(client):
    episodes_data = {
        "data": [{"id": 7, "title": "Episode 1", "duration": 1800, "description": "Intro", "imageUrl": "http://img.example.com"}]
    }
...
def test_search_track(client):
    data = {
        "results": {
            "tracks": [
                {"id": 99, "title": "Song", "artistName": "Artist", "albumName": "Album", "image": "http://img.example.com", "artistId": 5, "albumId": 10}
            ]
        }
    }
    with patch.object(client.session, "get", return_value=_mock_get(data)):
...
def test_search_artist(client):
    search_data = {"results": {"artists": [{"id": 3, "name": "The Band", "image": "http://img.example.com"}]}}
...
def test_search_playlist(client):
    data = {
        "results": {
            "playlists": [
                {"id": 55, "name": "Chill Vibes", "description": "Relax", "urls": {"web": "http://example.com", "image": "http://img.example.com"}}
            ]
        }
    }
    with patch.object(client.session, "get", return_value=_mock_get(data)):
